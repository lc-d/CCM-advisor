# Template scheda di analisi

Struttura fissa del documento di output. Non modificare l'ordine delle sezioni.

Inizia sempre definendo i valori del frontmatter in YML come segue 
---
ticker: [TICKER]
title: [nome-azienda]
model: [il modello AI attualmente utilizzato]
date: [data attuale es.: 2011-10-10T14:48:00.000+09:00]
draft: true
---

# [Nome azienda] ([TICKER]) — Scheda di analisi

## Panoramica

### Storia
[Breve storia dell'azienda dalla fondazione, in massimo 1000 caratteri]

### Ownership
[Elenco e analisi della ownership]

### Prodotti
[I prodotti principali: elenco con breve descrizione]

### Revenues stream
[Revenues stream]

### MOAT
[Valutazione del MOAT (se c'è un MOAT)]

### Potenziali concorrenti
[Elenco potenziali concorrenti]

## Conversazione con l'utente
[Analisi qualitativa emersa dalla fase uno (cerca nelle note se non c'è già una conversazione attiva) con particolare enfasi sulle domande poste dall'utente]

## Panoramica del business

[Modello di business, segmenti principali, posizionamento competitivo]

## Dati finanziari chiave

| Metrica           | Ultimo periodo | Anno precedente | Variazione YoY |
|-------------------|----------------|-----------------|----------------|
| Fatturato         |                |                 |                |
| Margine lordo     |                |                 |                |
| Margine operativo |                |                 |                |
| Utile netto       |                |                 |                |
| EPS               |                |                 |                |
| Free cash flow    |                |                 |                |

*Fonte: [documento e data]*

## Valutazione

| Multiplo       | Valore | Nota |
|----------------|--------|------|
| P/E (trailing) |        |      |
| P/E (forward)  |        |      |
| EV/EBITDA      |        |      |
| P/S            |        |      |
| Dividend yield |        |      |

## Bilancio e solidità finanziaria

[Liquidità, debito totale, debito/equity, scadenze di rifinanziamento rilevanti]

## Catalizzatori (prossimi 6-12 mesi)

- [Catalizzatore — fonte e data]

## Rischi principali

- [Rischio specifico all'azienda — fonte e data]

## Fonti consultate

- [Fonte con link e data di accesso]
